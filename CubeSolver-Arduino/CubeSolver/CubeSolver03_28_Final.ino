...
#define RPM 310//Sets speed of the steppers
#define fun 20//Delay after every turn
...
void setup() {
    ...
    //Enables 12V power on 'D8' on the ramps board to power LEDs
    pinMode(11, OUTPUT);
...
void checkmessage()
{
  if (message == 'Z')
  {
      ...
      case 'A': //U
        digitalWrite(ENABLE_X, LOW);
        stepper_X.rotate(QT);
        delay(delayfun (QT)); //Delay must be set so all the serial messaging doesn't interfere with interrupt
        digitalWrite(ENABLE_X, HIGH);
        break;
...
int delayfun (int deg)
{
  int del = (deg/((RPM*360)/ 60000) + fun);
