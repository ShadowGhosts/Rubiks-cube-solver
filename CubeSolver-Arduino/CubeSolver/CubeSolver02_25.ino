...
#define RPM 20 //Sets speed of the steppers
...
#define sd 2000//Delay used inbetween every stepper motor turn so LCD can keep up (sd = stepper delay)
...
void checkmessage()
{
  if (message == 'Z')
  {
      ...
      case 'A': //U
        stepper_X.rotate(QT);
        delay(sd); //Delay must be set so all the serial messaging doesn't interfere with interrupt
        break;
      ...
      case 'C': //L
        stepper_Z.rotate(QT);  //Cube turned slightly too much
        delay(sd);
        break;
